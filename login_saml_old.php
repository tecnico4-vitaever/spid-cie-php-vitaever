...
if(in_array($client_id, array_keys($clients)) && isset($_GET['redirect_uri'])) {

    $redirect_to_app = false;
    $redirect_uri = $_GET['redirect_uri'];

    if (isset($_GET['redirect_to_app'])) {
    ...
    if ($spidsdk->isAuthenticated()
        ...
        */
        ...
        $final_redirect = "$redirect_uri?token=" . urlencode($jwt) . "&is_spid_login=" . $isSpidLogin;

        if (!$redirect_to_app) {
            header("Location: $final_redirect");
        } else {
            $appLink = 'ntk-app://' . $final_redirect;
        ...
        $returnTo = $_SERVER['SCRIPT_URI'].'?idp='.$idp.'&redirect_uri='.$redirect_uri;
